...
impl DeterministicEquivRuntime for FixShellPhysical {
    ...
    fn create_blob_i64(data: u64) -> Self::Handle {
        ...
        __m256i::from(u8x32::from_array(buf))
    }
    ...
    fn is_blob(handle: Self::Handle) -> bool {
        let handle = FixHandle::unpack(handle.into());
        handle
...
pub extern "C" fn fixpoint_attach_tree(addr: *mut c_void, handle: __m256i) -> u64 {
    ...
    let mut entry = arca_entry {
        mode: __MODE_read_only,
        data: tree.clone().into_inner().as_raw() as usize,
        datatype: __TYPE_table,
    };
...
#[unsafe(no_mangle)]
#[target_feature(enable = "avx2")]
pub extern "C" fn arca_blob_to_handle(h: i64) -> __m256i {
    let mut buf = [0u8; 32];
    unsafe { arca_blob_read(h, 0, buf.as_mut_ptr(), 32) };
    __m256i::from(u8x32::from(buf))
}

#[unsafe(no_mangle)]
#[target_feature(enable = "avx2")]
pub extern "C" fn handle_to_arca_blob(h: __m256i) -> i64 {
    let h: u8x32 = h.into();
    unsafe { arca_blob_create(h.as_array().as_ptr(), 32) }
}
