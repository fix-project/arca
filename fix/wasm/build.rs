...
fn main() -> Result<()> {
    ...
    let mut generated = String::from("pub const ARTIFACTS: &[(&str, &[u8])] = &[\n");
    for f in std::fs::read_dir(".")? {
        let f = f?;
        let path = f.path();
        if path.extension() != Some(OsStr::new("wat")) {
            continue;
        }
        ...
        generated.push_str(&format!("    (\"{name}\", include_bytes!(\"{name}\")),\n"));
    }

    generated.push_str("];\n");
    std::fs::write(Path::new(&out_dir).join("artifacts.rs"), generated)?;
