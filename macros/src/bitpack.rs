    ...
    let pack_arms = variants.iter().map(|v| {
        ...
        quote! {
            #pat => {
                ...
                result &= !Self::TAGMASK;
                let field: &mut u64x4 = unsafe { core::mem::transmute( &mut result ) };
                field[3] |= (#index as u64) << (256 - #tag_bits);
                result
            }
        }
    });
    ...
    let output = quote! {
        ...
        impl BitPack for #name {
            ...
            fn unpack(content: u8x32) -> Self {
                ...
                let field: &u64x4 = unsafe { core::mem::transmute( &tag ) };
                let tag = field[3] >> (256 - #tag_bits);
                match tag {
                    #(#unpack_arms)*
                    _ => todo!()
                }
