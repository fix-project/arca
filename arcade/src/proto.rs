...
pub async fn read_request(
    mut data_file: Box<dyn File>,
) -> Result<(Message, Box<dyn File>), ParseError> {
    ...
    match first_byte[0] {
        b'R' => {
            ...
            let file_path = core::str::from_utf8(&file_path[..file_path_len])
                .unwrap()
                .trim();
        ...
        b'C' => {
            ...
            assert!(
                data_len as u32 == continuation_size,
                "Read data length does not match continuation size"
            );
            Ok((
        ...
        b'F' => {
            ...
            if file_size > 2 * 1024 * 1024 {
                ...
                while total_read < file_size as usize {
                    let chunk_size =
                        core::cmp::min(2 * 1024 * 1024, file_size as usize - total_read);
                    let data_len = data_file
                        .read(&mut file_data[total_read..total_read + chunk_size])
                        .await
                        .unwrap();
                    total_read += data_len;
                }
                ...
                if file_size != data_len as u32 {
                    log::error!(
                        "Read data length {} does not match file size {}",
                        data_len,
                        file_size
                    );
