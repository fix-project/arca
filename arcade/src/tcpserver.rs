...
impl Drop for ContinuationServer {
    fn drop(&mut self) {
        let mut f = pin! {self.continuations.send(None)};
        let _ = f.poll_unpin(&mut Context::from_waker(Waker::noop()));
    }
}
...
impl ContinuationClientTx {
    ...
    pub async fn request_to_run(&self, data: Vec<u8>) -> Result<(), Error> {
        let _ = self.continuation_request_future_send.send(Some(data)).await;
        Ok(())
    }
